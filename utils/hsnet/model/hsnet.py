...
class HypercorrSqueezeNetwork(nn.Module):
    def __init__(self, args, use_original_imgsize):#, backbone
        ...
        # self.backbone.eval()
        self.hpn_learner = HPNLearner(args, list(reversed(nbottlenecks[-3:])))
        self.cross_entropy_loss = nn.CrossEntropyLoss()

    def forward(self, feats, old_feats):
        with torch.no_grad():            
            corr = Correlation.multilayer_correlation(feats, old_feats, self.stack_ids)

        # liangsen = np.array(corr)
        # print("corr's shape is : {}".format(liangsen.shape))
