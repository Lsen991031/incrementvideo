...
class HPNLearner(nn.Module):
    def __init__(self, args, inch):
        ...
        #                         nn.ReLU())        
        och1, och2 = 128, 512
        self.decoder_ucf = nn.Sequential(nn.Conv2d(128, och1, (3, 3), padding=0, stride=2, bias=True),
                        nn.BatchNorm2d(och1),
                        nn.ReLU(),
                        nn.Conv2d(och1, och2, (3, 3), padding=1, stride=2, bias=True),
                        nn.BatchNorm2d(och2),
    ...
    def forward(self, hypercorr_pyramid):
        ...
        if self.dataset == 'ucf101':
            ...
            # channel_fc = hypercorr_decoded.view(16, -1)
            # channel_fc = self.fc(channel_fc)
            # channel_fc = self.softmax(channel_fc)
            # channel_fc = channel_fc.view(int(bsz/8), 8*512).mean(dim=-2)
            # channel_fc = channel_fc.view(8, 512)
            # print("channel_fc----------shape is : {}".format(channel_fc.shape))
            # print("hypercorr_decoded----------shape is : {}".format(hypercorr_decoded.shape))
