...
class HPNLearner(nn.Module):
    ...
    def forward(self, hypercorr_pyramid):
        ...
        if self.dataset == 'ucf101':
            ...
            # hy1 = self.avgpool(hypercorr_decoded)
            # hy1 = hy1.view(bsz, 2048)
            # print("hy1---------- is : {}".format(hy1))
            # hy2 = self.fc(hy1)
            # # print("hy2---------- is : {}".format(hy2))
            # hy2 = self.softmax(hy2)
            # hy2 = hy2.view(int(bsz/8), 8*2048).mean(dim=-2)
            # print("hy22---------- is : {}".format(hy2))

        hypercorr_decoded1 = hypercorr_decoded1 * self.scaler

        # print("decoded1---------- is : {}".format(hypercorr_decoded1.shape))
        hypercorr_decoded2 = hypercorr_decoded2 * self.scaler
        # hypercorr_decoded2 = hy2 * 10000
        print("hypercorr_decoded2---------- is : {}".format(hypercorr_decoded2))
