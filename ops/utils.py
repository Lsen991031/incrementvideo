...
def accuracy(args, output, target, topk=(1,)):
    ...
    for k in topk:
        ...
        correct_k = reduce_value(correct_k, True) 
...
def load_exemplars(args, device):
    # '''DDP parameter'''
    # rank = args.rank
    # device = torch.device(args.gpu)

    filename = '{}/{}/{}/{}/{:03d}/exemplars.pth'.format(args.root_model, args.dataset, str(args.init_task), str(args.nb_class), args.exp)
    exemplar_dict = torch.load(filename, map_location=device)
...
def load_importance(args):
    ...
    filename = '{}/{}/{}/{}/{:03d}/importance.pth'.format(args.root_model, args.dataset, str(args.init_task), str(args.nb_class), args.exp)
    importance_dict = torch.load(filename, map_location=device)
