...
class TSN(nn.Module):
    def __init__(self, args, num_class, new_length=None, before_softmax=True,
                 crop_num=1, print_spec=True, fc_lr5=False, apply_hooks=False,
                 age=0, training=False, cur_task_size=0,
                 fine_tune=False):
        ...
        self.importance = args.use_importance
    ...
    def _prepare_tsn(self, cur_task_size): # num_class = old_class for age > 0
        ...
        elif self.fc_type == 'linear': # CosineLinear has its own initializer
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            self.base_model = getattr(models, base_model)(True if self.pretrain == 'imagenet' else False,
                    self.remove_last_relu, self.num_segments)
            if self.is_shift:
    ...
    def get_optim_policies(self):
        ...
        for m in self.modules():
            ...
            elif isinstance(m, CosineLinear):
                ps = list(m.parameters())
            ...
            elif isinstance(m, SplitCosineLinear):
                ps = list(m.parameters())
                for p in ps:
    ...
    def forward(self, input, target=None, no_reshape=False, only_feat=False, t_div=False):
        outputs = {}
        if not no_reshape:
        ...
        feature_conv5 = feature_conv5.view(-1,self.num_segments,feature_conv5.size()[-1])

        if self.dropout > 0: # dropout = 0.8
            if self.importance:
        ...
        if self.reshape:
            ...
            outputs['preds'] = output.squeeze(1)
            outputs['feat'] = feature_conv5
    ...
    def _get_div(self, int_feat):
        ...
        for i in range(num_layers):
            ...
            loss_i = torch.norm(R-I, p='fro', dim=(2,3))/num_pairs #F.frobenius_norm(R-I)
            loss_i = loss_i.sum()
