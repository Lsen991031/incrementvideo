...
def _train(args, train_loader, model, criterion, optimizer, epoch, age, lambda_0=[0.5,0.5], model_old=None):
    ...
    for i, (input, target, _) in enumerate(train_loader):
        ...
        prec1, prec5 = accuracy(args, preds.data, target, topk=(1, 5))
...
def _validate(args, val_loader, model, criterion, age):
    ...
    with torch.no_grad():
        for i, (input, target, _) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            prec1, prec5 = accuracy(args, output.data, target, topk=(1, 5))
...
def train_task(args, age, total_task, current_head, class_indexer, prefix=None):
    ...
    print("cbf.device is : {}".format(device))
    sd = sd['state_dict']
    state_dict = dict()
    for k, v in sd.items():
        state_dict[k[7:]] = v
